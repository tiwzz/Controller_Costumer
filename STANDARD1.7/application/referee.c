...
void init_referee_struct_data(void)
{
    ...
    memset(&rx_data_t, 0, sizeof(Controller_t_Rx)); // 图传链路
}
...
void referee_data_solve(uint8_t *frame)
{
    ...
    case IDCustomDatarx:
    {
        memcpy(&rx_data_t, frame + index, sizeof(Controller_t_Rx)); // 图传链路
    }
	break;
    default:
    {
...
void customcontroller_data_pack_send(uint8_t *p_data, uint16_t len)
{
    ...
    uint16_t frame_length = 39; // 数据帧长度

    customcontroller_tx.frame_header.SOF = 0xA5;                                          // 数据帧起始字节
    customcontroller_tx.frame_header.data_length = len;                                   // 数据中data的长度
    customcontroller_tx.frame_header.seq = seq++;                                           // 包序号
    append_CRC8_check_sum((uint8_t *)(&customcontroller_tx.frame_header), frameheader_len); // 帧头校验CRC8

    customcontroller_tx.IDCustomData = 0x0302; // 命令码
